...
# ## original version with more output
...
#         if logger.in_venv:
#             click.echo(f"   {logger.log_file} (venv-specific)")
#             click.echo("   🎯 Each virtual environment has its own log!")
#         else:
#             click.echo(f"   {logger.log_file}")
        
#         click.echo(f"\n🎉 {random.choice(MOTIVATIONAL_PHRASES)}")
#         # click.echo("\n" + "="*60)
#     #     click.echo("⚠️  IMPORTANT: Run this to activate in current shell:")
#     #     click.echo("="*60)
    
#     # # Output the source command that needs to be run
#     # click.echo(f"source {hook_file}")
    
#     if not quiet:
#         click.echo("="*60)
#         click.echo("\n💡 Pro tip: Use this for auto-activation:")
